...
function A3_GUI_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    Params.Speed = "V500";  
    Params.Mode = " ";
    Params.Frame1 = "Table";        % Target Frame
    Params.Frame2 = "Table";        % Destination Frame
...
function ProgShutDownBtn_Callback(hObject, eventdata, handles)
    ...
    switch Params.FrameCurrent
        case "Table"
            Pose.Target = [str2double(get(handles.EEX,"String"))-175,str2double(get(handles.EEY,"String")),50];
            CommandQueue = [CommandQueue,"SetTarget"];
            Joints.Target = [-90,0,0,0,0,0];
...
function BP1BP2Btn_Callback(hObject, eventdata, handles)
    ...
    switch Params.FrameCurrent
        ...
        case "Table"
            ...
            % Find desired target block position
            fromBP = cellstr(get(handles.FromBPList,'String'));
            fromBP = fromBP{get(handles.FromBPList,'Value')};

            % Find it's coordinates
            BP1 = BlockPoints(str2num(fromBP),:);

            % Set target to those coordinates
            Pose.Target = [BP1(1),BP1(2),40];
            % Send command to set pose
            CommandQueue = [CommandQueue,"SetTarget"];

            Pose.Avoid2 = [BP1(1),BP1(2),BP1(3)];
...
function BPCONVBtn_Callback(hObject, eventdata, handles)
    ...
    switch Params.FrameCurrent
        ...
        case "Conv"
            ...
            Pose.Avoid2 = [BP1(1),BP1(2),50];
            CommandQueue = [CommandQueue,"SetAvoid2"];
            
            Pose.Target = [BP1(1),BP1(2),BP1(3)];
            CommandQueue = [CommandQueue,"SetTarget"];
            
            % Send Command to turn solenoid and pump on
            CommandQueue = [CommandQueue,"VacOn"];
            
            Pose.Avoid3 = [BP1(1),BP1(2),50];
            CommandQueue = [CommandQueue,"SetAvoid3"];
            
            Joints.Destination = [90,30,0,0,0,0];
            CommandQueue = [CommandQueue,"SetDestJoints"];
            
            % Set Frame of Reference to the table frame for positioning
            Params.Frame2 = "Conv";
            CommandQueue = [CommandQueue,"SetDestFrame"];
            
            % Set Destination coordinates (maintain it as conveyor home for the
            % moment
            Pose.Destination = [0,0,0];
            ...
            Params.FrameCurrent = Params.Frame2;
...
% --- Executes on selection change in FromBPList.
function FromBPList_Callback(hObject, eventdata, handles)
    % Dont need this as we will be obtaining parameters from button press;
end

% --- Executes during object creation, after setting all properties.
function FromBPList_CreateFcn(hObject, eventdata, handles)
    if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
    end
end

% --- Executes on selection change in ToBPList.
function ToBPList_Callback(hObject, eventdata, handles)
    % Dont need this as we will be obtaining parameters from button press;
end

% --- Executes during object creation, after setting all properties.
function ToBPList_CreateFcn(hObject, eventdata, handles)
    if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
    end
end
