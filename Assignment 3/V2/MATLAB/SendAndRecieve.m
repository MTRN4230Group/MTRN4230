        ...
        if(CommSocket.STATUS == true)
            ...
            if ~isempty(CommandQueue) && (CommSocket.STATUS == true) && (Err2Resolve == 0)
                ...
                switch nextCommand
                    ...
                    case "SetPose"
                        try
                            CommSocket.setPose(Pose(1,:));
                            CommandMsg = "Moving Robot To: ";
                            CommandMsg2 = sprintf('%.2f, %.2f, %.2f',Pose(1,:));
                ...
                case "SetFrame"
                    try
                        CommSocket.setFrame(Params.Frame(1));
                        if(CommSocket.STATUS)
                            CommandMsg = sprintf('Setting Frame: %s', Params.Frame);
                            SentCommands{length(SentCommands)+1} = CommandMsg;
                            set(Handles.TXcommands,'String',SentCommands);
                            set(Handles.TXcommands,'ListboxTop',length(SentCommands));
                            Params.Frame = Params.Frame(2:end);
                        end
                    catch
                    end
                ...
                case "SetJoints"
                    try
                        CommSocket.setJoints(Joints(1,:));
                        CommandMsg = sprintf('Setting Joints: %s', Joints(1,:));
                        if(CommSocket.STATUS)
                            SentCommands{length(SentCommands)+1} = CommandMsg;
                            set(Handles.TXcommands,'String',SentCommands);
                            set(Handles.TXcommands,'ListboxTop',length(SentCommands));
