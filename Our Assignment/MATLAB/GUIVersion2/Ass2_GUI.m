function varargout = Ass2_GUI(varargin)
    ...
    % Last Modified by GUIDE v2.5 24-Aug-2018 14:05:03
...
% function trialnew(handles)
%         while(1)
%         val = get(handles.JogXPlusBtn,'Value');
%         set(handles.texttrial,'String','newnew');
%     end
% end
...
% --- Executes on selection change in JogSpdPopup.
function JogSpdPopup_Callback(hObject, eventdata, handles)
    % hObject    handle to JogSpdPopup (see GCBO)
    % eventdata  reserved - to be defined in a future version of MATLAB
    % handles    structure with handles and user data (see GUIDATA)
    % Hints: contents = cellstr(get(hObject,'String')) returns JogSpdPopup contents as cell array
    %        contents{get(hObject,'Value')} returns selected item from JogSpdPopup
end

% --- Executes during object creation, after setting all properties.
function JogSpdPopup_CreateFcn(hObject, eventdata, handles)
    % hObject    handle to JogSpdPopup (see GCBO)
    % eventdata  reserved - to be defined in a future version of MATLAB
    % handles    empty - handles not created until after all CreateFcns called
    % Hint: popupmenu controls usually have a white background on Windows.
